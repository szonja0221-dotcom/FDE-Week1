...
**Version:** 1.0.1
**Status:** Loop 1 Refactor — Fixes Applied
...
CREATE TABLE permit_application (
    ...
    status                  VARCHAR(40)     NOT NULL DEFAULT 'SUBMITTED' CHECK (status IN (
                                ...
                                'ESCALATED_TO_SENIOR','SPOT_CHECK_PENDING',
    ...
    target_review_date      TIMESTAMPTZ,                     -- system-set when triage_category = ROUTINE; never client-supplied
    sla_status              VARCHAR(20)     DEFAULT NULL CHECK (sla_status IN (
                                'ON_TRACK','SLA_AT_RISK','SLA_BREACHED'
                            )),
    spot_check_required     BOOLEAN         NOT NULL DEFAULT FALSE,
    ...
    CONSTRAINT chk_sla_routine_only
        CHECK (sla_status IS NULL OR triage_category = 'ROUTINE'),
    CONSTRAINT chk_target_review_date_routine_only
        CHECK (target_review_date IS NULL OR triage_category = 'ROUTINE'),
    CONSTRAINT chk_spot_check_pending_requires_flag
        CHECK (status != 'SPOT_CHECK_PENDING' OR spot_check_required = TRUE)
);
```
...
CREATE TABLE audit_log (
    ...
    event_type              VARCHAR(50)     NOT NULL CHECK (event_type IN (
                                ...
                                'SPOT_CHECK_CONFIRMED','SPOT_CHECK_OVERRIDE',
                                'TRIAGE_CATEGORY_OVERRIDE','REVIEWER_OVERRODE_DEFICIENCIES',
                                'SLA_STATUS_CHANGE'
                            )),
    old_value               TEXT,
    new_value               TEXT,
    ...
    └──[Agent: completeness check]──▶ INTAKE_REVIEW
              ...
              RETURNED_INCOMPLETE                        TRIAGE_PENDING
                   (terminal*)                                   │
                    ...
                    ┌──────────────────────────────┼───────────────────┐   │
                    │                              │                   │   │
    [spot_check=false]▼         [spot_check=true]  ▼                   ▼   │
             APPROVED           SPOT_CHECK_PENDING  REJECTED   PENDING_APPLICANT_INFO
          (terminal*)                   │        (terminal*)          │
                            ┌───────────┴───────────┐         [info received]
                            │                       │                 │
                [confirms]  ▼           [overrides] ▼         UNDER_REVIEW ─┘
                        APPROVED          TRIAGE_PENDING
                     (terminal*)        (re-triage as COMPLEX_SENIOR)
                    │
...
| T-04 | `TRIAGE_PENDING` | `UNDER_REVIEW` | `triage_category` NOT NULL; `agent_recommendation` NOT NULL; `agent_confidence_score >= AGENT_CONFIDENCE_THRESHOLD (0.80)`; if `triage_category = ROUTINE` then additionally `agent_confidence_score >= ROUTINE_CONFIDENCE_THRESHOLD (0.95)`; all applied `rule_set` records have `active = true AND effective_date <= submitted_at AND (sunset_date IS NULL OR sunset_date > submitted_at)`; `assigned_reviewer_id` NOT NULL; assigned reviewer `active = true`, eligible for `triage_category`, `current_workload < max_workload`, licensed for application jurisdiction | **Agent Alone** (assignment) then **Human** (acknowledgment) |
| T-05 | `UNDER_REVIEW` | `APPROVED` | `reviewer_decision = APPROVED`; `reviewer_notes` NOT NULL; `spot_check_required = false`; `decision_session_id` references a valid, non-expired `auth_session` for `assigned_reviewer_id`; `decided_at` NOT NULL | **Human Only** |
...
| T-13 | `UNDER_REVIEW` | `SPOT_CHECK_PENDING` | `reviewer_decision = APPROVED`; `spot_check_required = true`; `decision_session_id` valid; `decided_at` NOT NULL; a `SENIOR_EXAMINER` with `current_workload < max_workload` must be available | **System** (auto on approval when spot-check flag is set) |
| T-14 | `SPOT_CHECK_PENDING` | `APPROVED` | Senior examiner review complete; no misclassification found; `reviewer_notes` NOT NULL; `decision_session_id` for senior examiner valid; log `SPOT_CHECK_CONFIRMED` event | **Human Only** (SENIOR_EXAMINER) |
| T-15 | `SPOT_CHECK_PENDING` | `TRIAGE_PENDING` | Senior examiner finds misclassification; `triage_category` reverted to correct value; log `SPOT_CHECK_OVERRIDE` with `old_value = ROUTINE`, `new_value = <corrected category>`, and reason | **Human Only** (SENIOR_EXAMINER) |
...
| ID | Condition | Expected Behavior |
|---|---|---|
| FM-R-01 | `agent_confidence_score = 0.79` (below `AGENT_CONFIDENCE_THRESHOLD` 0.80) | Override `triage_category` to `COMPLEX_SENIOR`; set `agent_recommendation = NEEDS_HUMAN_REVIEW`; do not route as ROUTINE regardless of other signals. Log with reason `LOW_CONFIDENCE_GENERAL`. |
...
| FM-R-03 | `agent_confidence_score = 0.88` (≥ 0.80 but < `ROUTINE_CONFIDENCE_THRESHOLD` 0.95); all other ROUTINE criteria met | Agent must NOT classify as ROUTINE (Conservative Triage Policy). If a correctable deficiency is present: classify `DEFICIENT`. If no correctable issue is identifiable: classify `COMPLEX_SENIOR`. Log with reason `LOW_CONFIDENCE_ROUTINE`. T-04 blocks promotion to `UNDER_REVIEW` as `ROUTINE` when this condition applies. |
...
| `target_review_date` | `permit_application` | Set when `triage_category = ROUTINE` is first assigned; never updated |
...
| `ROUTINE_CONFIDENCE_THRESHOLD` | `DECIMAL` | `0.95` | Minimum confidence score to classify as ROUTINE; score in [0.80, 0.95) routes to DEFICIENT or COMPLEX_SENIOR per Conservative Triage Policy |
| `SPOT_CHECK_RATE` | `DECIMAL` | `0.05` | CSPRNG threshold for SPOT_CHECK_PENDING sampling on ROUTINE approvals; range [0.0, 1.0] |
| `SLA_AT_RISK_HOURS` | `INTEGER` | `24` | Hours after `submitted_at` before ROUTINE application is flagged `SLA_AT_RISK` |
| `SLA_BREACH_BUSINESS_DAYS` | `INTEGER` | `2` | Business days after `submitted_at` before `target_review_date`; also the SLA breach threshold |
